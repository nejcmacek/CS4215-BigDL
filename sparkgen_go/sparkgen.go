...
func logger(j *job, cmd *exec.Cmd, lid float32) {
	...
	if debug {
		...
		log.Printf("[LOG-DONE] %d,%d,%t,%f,%f,%f", j.class, j.id, err == nil, j.gen, j.start, j.end)
	}
...
func selectJob(max int, remove bool) *job {
	...
	for i := 0; i < max; i++ {
		if debug {
			log.Printf("[SEL] Length of Queue %d is %d", i, queues[i].Len())
		}
		if queues[i].Len() > 0 {
			e := queues[i].Front()
			j = e.Value.(*job)
			...
			if debug {
				log.Printf("[SEL] Selected job %d from Queue %d", j.id, i)
			}
			break
		}
	}
...
func scheduler() {
	...
	for run == 1 {
		...
		case <- gen:
			...
			if runningJobs.Len() < numberOfNodes { // No job running -> dispatch next job
		...
		case <- done:
            ...
            for e := runningJobs.Front(); e != nil; e = next {
                next = e.Next()

                if e.Value.(*job).end > 0 {
                    runningJobs.Remove(e)
                }
            }
